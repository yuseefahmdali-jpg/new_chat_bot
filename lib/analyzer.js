...
function detectSentiment(text) {
  const negative = containsAny(text, NEGATIVE_WORDS);
  const positive = containsAny(text, POSITIVE_WORDS);
...
function extractEntities(rawText = '') {
  ...
  return {
    emails: [...text.matchAll(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi)].map((match) => match[0]),
    phones: [...text.matchAll(/(?:\+?\d[\d\s-]{7,}\d)/g)].map((match) => match[0].trim()),
    urls: [...text.matchAll(/https?:\/\/[^\s]+/g)].map((match) => match[0]),
    orderIds: [...text.matchAll(/#?[A-Z]{0,3}\d{4,}/g)].map((match) => match[0]),
    dates: [...text.matchAll(/\b(?:\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})\b/g)].map((match) => match[0]),
...
const FORBIDDEN_SUGGESTIONS = ['العودة للمحادثة الطبيعية', 'فتح تذكرة دعم', 'التحدث مع موظف'];

function analyzeContact(payload) {
  ...
  const summaryText = summarize(message);
...
function textToKeywordSet(text = '') {
  return new Set(normalize(text).split(/[^a-zA-Z\u0600-\u06FF0-9]+/).filter((item) => item.length > 2));
}
...
async function retrieveMemories(sessionId, message, options = {}) {
  try {
...
async function generateChatReply(sessionId, rawMessage, store, options = {}) {
  ...
  const memories = await retrieveMemories(sessionId, message, { userId: options.userId || null });

  let coreReply = CHAT_REPLY[detectedIntent] || CHAT_REPLY.general;
  if (memories.length > 0) {
    const preview = memories[0].content.slice(0, 60).trim();
    coreReply = `(استناداً لتواصلنا السابق: "${preview}...") ${coreReply}`;
  }

  return {
    reply: coreReply,
    answer: coreReply,
    suggestions: INTENT_SUGGESTIONS[detectedIntent] || INTENT_SUGGESTIONS.general,
    detectedIntent,
