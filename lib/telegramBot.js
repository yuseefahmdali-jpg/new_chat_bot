...
if (!BOT_TOKEN) {
  console.warn('[telegramBot] TELEGRAM_BOT_TOKEN غير مضبوط — تم تعطيل بوت تيليغرام');
  module.exports = null;
} else {
  if (!ADMIN_CHAT_ID) {
...
bot.onText(/^\/reply\s+(\S+)\s+([\s\S]+)$/, async (msg, match) => {
  ...
  const ticketOwner = await getUserById(ticket.user_id);

  await saveTicketMessage(ticketId, 'admin', replyText, {
    userId: ticket.user_id,
    sessionId: ticketOwner && ticketOwner.sessionId ? ticketOwner.sessionId : null,
    mirrorToMessages: Boolean(ticketOwner && ticketOwner.sessionId),
    meta: { ticketId, sender: 'admin', source: 'telegram-admin' },
  });
...
bot.on('message', async (msg) => {
  ...
  try {
    ...
    if (!ticket) {
      ticket = await createTicket(user.id, name, user.sessionId || null);
      await safeSend(
        chatId,
    ...
    await saveTicketMessage(ticket.id, 'user', text, {
      ...
      mirrorToMessages: Boolean(user.sessionId),
      meta: { ticketId: ticket.id, sender: 'user', source: 'telegram-user' },
    });
    ...
    await safeSend(chatId, '✅ تم استلام رسالتك. سيرد عليك أحد موظفي الدعم قريباً.');
  } catch (err) {
